...
(async function(){
  ...
  note.textContent='Falltexte in einfachem Deutsch · Weitere Texte teilweise automatisch übersetzt. Medizinische Inhalte bitte fachlich prüfen.';
  ...
  function translate(text){const t=norm(text);let m;if(m=t.match(/^(.*?) · (Starter|Building|Stretch) · (\d+) min$/))return `${m[1]} · ${{Starter:'Einfach',Building:'Mittel',Stretch:'Fortgeschritten'}[m[2]]} · ${m[3]} min`;if(dictionary[t])return dictionary[t];
